...
export default function ProjectView(props) {
  ...
  const updateSentence = async (sentence_id) => {
    const response = await axios.post(
      `http://localhost:8000/wikipedia/sentence/update`,
      {
        sentence_id: sentence_id,
        translated_sentence: state["sentences"].find(
          (el) => el.sentence_id === sentence_id
        )["translated_sentence"],
      }
    );
  ...
  const onChange = (e, sentence_id) => {
    ...
    setState({
      ...
      sentences: sentences.map((el) => {
        return el.sentence_id === sentence_id
  ...
  const fetchProject = async () => {
    const response = await axios.get(
      "http://localhost:8000/wikipedia/project/get/" + projectID
    );
    if (response.status === 200) {
                ...
                <Grid templateColumns="repeat(12,1fr)">
                  <GridItem colSpan={9}>
                    <ReactTransliterate
                      ...
                      onChangeText={(e) => {
                        onChange(e, element["sentence_id"]);
                      }}
                      style={{ width: "100%" }}
                  ...
                  <GridItem colSpan={3}>
                    <Button
                      onClick={() => updateSentence(element["sentence_id"])}
                    >
                      Update
                    </Button>
                  </GridItem>
