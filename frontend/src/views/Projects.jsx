...
function Projects(props) {
  ...
  const createProject = async () => {
     await axios.post(
      "http://localhost:8000/wikipedia/project/create",
      newProject
    );
    fetchProjects();
    onClose();
  ...
  {
    await axios.delete("http://localhost:8000/wikipedia/project/delete/"+project_id)
    fetchProjects();
  }
  const fetchProjects = async () => {
    const response = await axios.get(
      "http://localhost:8000/wikipedia/project/get"
    );
        ...
        {state &&
          state.map((element, index) => (
            <GridItem
              ...
              boxShadow={"lg"}
            >
              <Text fontSize={"xl"} fontWeight='bold'>{element.wiki_title}</Text>
              <Text fontSize={"lg"}>{element.language_name}</Text>
              {/* <Text fontSize={"xl"}>
                {
                  props.languages.filter(function (ele) {
                    return ele.language_id == element.fk_target_lang;
                  })["name"]
                }
              </Text> */}
              <Link to={`project/${element.project_id}`}>
                <Button mt="5%">Open Project</Button>
              </Link>
              <Button mt="5%" ml="3%" colorScheme="red" onClick={() => deleteProject(element.project_id)}>
                <SmallCloseIcon />
              </Button>
            </GridItem>
          ))}
            ...
            >
              {props.languages &&
                props.languages.map((element) => (
                  <option value={element.language_id}>{element.name}</option>
                ))}
            </Select>
          </ModalBody>
          <ModalFooter>
